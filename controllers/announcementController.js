...
export const createAnnouncement = async (req, res) => {
    try {
        ...
        if (user.role === "admin") {
            if (
                user.position === "Chief Warden" ||
                user.position === "Junior Assistant"
            ) {
                if (type !== "Common") {
                    return res
                        .status(403)
                        .json({
                            error: "Chief Warden and Junior Assistant can only announce as 'Common' type.",
                        });
                }
                ...
                if (!["Common", "Hostel", "Worker"].includes(type)) {
                    return res
                        .status(400)
                        .json({
                            error: "Wardens can only announce as 'Common', 'Hostel', or 'Worker'.",
                        });
                }
                ...
                return res
                    .status(403)
                    .json({ error: "Unauthorized admin position." });
            }
        }
        // --- RBAC: Rules for Workers ---
            ...
            if (type !== "Worker") {
                return res
                    .status(403)
                    .json({
                        error: "Workers can only create 'Worker' type announcements.",
                    });
            }
            finalHostelName = user.hostel_name;
        }
        // --- Deny Others (Students) ---
            ...
            return res
                .status(403)
                .json({
                    error: "Only admins and workers can create announcements.",
                });
        }
        ...
        if (error.code === "23503") {
            return res
                .status(400)
                .json({
                    error: "Database constraint error. Ensure you dropped the fk_announcements_admin constraint to allow workers to announce.",
                });
        }
        res.status(500).json({
            error: "Server error while creating announcement.",
        });
    }
};
