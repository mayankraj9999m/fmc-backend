...
const verifyWarden = (req, res, next) => {
    ...
    if (position !== "Hostel Warden" && position !== "Associate Warden") {
        return res
            .status(403)
            .json({ error: "Access denied. Warden authorization required." });
    }

    if (!hostel_name) {
        return res
            .status(403)
            .json({ error: "No hostel assigned to your account." });
    }
...
router.post("/workers", async (req, res) => {
    try {
        const { name, email, phone_no, gender, department, sub_work_category } =
            req.body;
...
router.put("/workers/:id", async (req, res) => {
    try {
        ...
        const checkOwnership = await pool.query(
            "SELECT id FROM workers WHERE id = $1 AND hostel_name = $2",
            [id, req.wardenHostel],
        );
        if (checkOwnership.rows.length === 0) {
            return res
                .status(404)
                .json({ error: "Worker not found in your assigned hostel." });
        }
        ...
        if (password && password.trim() !== "") {
            ...
            updateQuery +=
                ", password_hash = $7 WHERE id = $8 RETURNING id, name, email, department";
            values.push(hashedPassword, id);
        } else {
            updateQuery +=
                " WHERE id = $7 RETURNING id, name, email, department";
            values.push(id);
        }
